...
import { AGING_BUCKETS, getCustomerAging, type AgingBucket } from '@/lib/reports';
            ...
            <div className="scroll-touch overflow-x-auto rounded-xl border border-border-app bg-surface">
              ...
              {report.rows.map((row) => (
                ...
                >
                  ...
                  <MoneyDisplay amount={row.total} tone="income" className="text-right text-xs font-semibold" />
                </Link>
              ))}
              <div className="grid min-w-[560px] grid-cols-[1fr_repeat(4,minmax(0,4.5rem))_minmax(0,5rem)] gap-1 bg-app px-3 py-3 font-semibold">
                ...
                <MoneyDisplay amount={report.grandTotal} tone="income" className="text-right text-xs" />
              </div>
            </div>
          </div>
        )}
